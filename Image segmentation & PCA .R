...
mean(USArrests$Rape)
apply(USArrests,2,mean) # 2 used for mean column wise
apply(USArrests,2,var) # varaince of each column
pr.out<- prcomp(USArrests,scale = TRUE) # SCALE = TRUE to scale each varable 
names(pr.out)
pr.out$scale
pr.out$center
pr.out$sdev
